# 1 "../Device/Display/drv_pdma.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 418 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../Device/Display/drv_pdma.c" 2
...
# 10 "../Device/Display/drv_pdma.c" 2
...
# 11 "../Device/Display/drv_pdma.c" 2
...
# 19 "C:\\Users\\antia\\AppData\\Local\\Keil_v5\\ARM\\ARMCLANG\\bin\\..\\include\\inttypes.h" 3
...
# 20 "C:\\Users\\antia\\AppData\\Local\\Keil_v5\\ARM\\ARMCLANG\\bin\\..\\include\\inttypes.h" 2 3
...
# 13 "../Device/Display/drv_pdma.c" 2
# 1 "../Device/Display\\nu_bitutil.h" 1
# 66 "../Device/Display\\nu_bitutil.h"
static inline int nu_clz(uint32_t x)
{
    return x ? __clz(x) : 32;
}
...
static inline int nu_clo(uint32_t x)
{
    return nu_clz(~x);
}
...
static inline int nu_ctz(uint32_t x)
{
    int c = 32;
...
static inline int nu_cto(uint32_t x)
{
    return nu_ctz(~x);
}


static inline uint16_t nu_get16_le(const uint8_t *pos)
{
    uint16_t val;
...
static inline void nu_set16_le(uint8_t *pos, uint16_t val)
{
    *pos ++ = val & 0xFF;
    *pos = val >> 8;
}
...
static inline uint32_t nu_get24_le(const uint8_t *pos)
{
    uint32_t val;
...
static inline void nu_set24_le(uint8_t *pos, uint32_t val)
{
    *pos ++ = val & 0xFF;
    *pos ++ = (val >> 8) & 0xFF;
    *pos ++ = (val >> 16) & 0xFF;
...
static inline uint16_t nu_get16_be(const uint8_t *pos)
{
    uint16_t val;
...
static inline void nu_set16_be(uint8_t *pos, uint16_t val)
{
    *pos ++ = val >> 8;
    *pos = (val & 0xFF);
}
...
static inline uint32_t nu_get24_be(const uint8_t *pos)
{
    uint32_t val;
...
static inline void nu_set24_be(uint8_t *pos, uint32_t val)
{
    *pos ++ = val >> 16;
    *pos ++ = val >> 8;
    *pos ++ = (val & 0xFF);
...
static inline uint32_t nu_get32_be(const uint8_t *pos)
{
    uint32_t val;
...
static inline void nu_set32_be(uint8_t *pos, uint32_t val)
{
    *pos ++ = val >> 24;
    *pos ++ = val >> 16;
    *pos ++ = val >> 8;
...
# 14 "../Device/Display/drv_pdma.c" 2
# 1 "../Device/Display\\drv_pdma.h" 1
# 35 "../Device/Display\\drv_pdma.h"
typedef enum
{
...
# 15 "../Device/Display/drv_pdma.c" 2
# 31 "../Device/Display/drv_pdma.c"
enum
{
    PDMA_START = -1,
    PDMA0_IDX,
...
static nu_pdma_chn_t nu_pdma_chn_arr[(PDMA_CNT*16UL)];
...
const static struct nu_module nu_pdma_arr[] =
{
    {
        .name = "pdma0",
        .m_pvBase = (void *)((PDMA_T *) ((((uint32_t) 0x40000000UL) + 0x00200000UL) + 0x00000UL)),
        .u32RstId = ((0x290UL<<20) | (0)),
        .eIRQn = PDMA0_IRQn
    },
    {
        .name = "pdma1",
        .m_pvBase = (void *)((PDMA_T *) ((((uint32_t) 0x40000000UL) + 0x00200000UL) + 0x01000UL)),
        .u32RstId = ((0x290UL<<20) | (1)),
        .eIRQn = PDMA1_IRQn
    },
};
...
static void *nvt_malloc_align(uint32_t size, uint32_t align, void **unalign_ptr)
{
    ...
    align_size = (((size) + (sizeof(void *)) - 1) & ~((sizeof(void *)) - 1)) + align;

    if ((ptr = malloc(align_size)) != 0)
    {
        void *align_ptr;
...
static const nu_pdma_periph_ctl_t g_nu_pdma_peripheral_ctl_pool[ ] =
{
    ...
    { 0UL, eMemCtl_SrcInc_DstInc },
};


static struct nu_pdma_memfun_actor nu_pdma_memfun_actor_arr[(4)];


static DSCT_T nu_pdma_sgtbl_arr[(16)] = { 0 };
static uint32_t nu_pdma_sgtbl_token[((((16)) + (32) - 1) & ~((32) - 1)) / 32];

static int nu_pdma_check_is_nonallocated(uint32_t u32ChnId)
{
    uint32_t mod_idx = ((u32ChnId)/16UL);
    do { if(!(mod_idx < PDMA_CNT)) { printf("Asserted at expression: %s", "mod_idx < PDMA_CNT"); while(1); } } while(0);
    return !(nu_pdma_chn_mask_arr[mod_idx] & (1 << ((u32ChnId)%16UL)));
}
...
static int nu_pdma_peripheral_set(uint32_t u32PeriphType)
{
    ...
    while (idx < ( sizeof(g_nu_pdma_peripheral_ctl_pool) / sizeof(g_nu_pdma_peripheral_ctl_pool[0]) ))
    {
...
static void nu_pdma_periph_ctrl_fill(int i32ChannID, int i32CtlPoolIdx)
{
    nu_pdma_chn_t *psPdmaChann = &nu_pdma_chn_arr[i32ChannID - (0)];
...
static void nu_pdma_init(void)
{
    ...
    for (i = (PDMA_START + 1); i < PDMA_CNT; i++)
    {
        ...
        nu_pdma_chn_mask_arr[i] = ~((((1ul<<16UL)-1) << (0)));

        SYS_ResetModule(nu_pdma_arr[i].u32RstId);


        PDMA_Open(psPDMA, ((1ul<<16UL)-1));
        PDMA_Close(psPDMA);
        ...
        __NVIC_EnableIRQ((IRQn_Type)nu_pdma_arr[i].eIRQn);
    ...
    {
        latest = ((16)) / 32;
        nu_pdma_sgtbl_token[latest] ^= ~((1 << ((16) % 32)) - 1) ;
    }
...
{
    PDMA_T *PDMA = (PDMA_T *)((((i32ChannID)/16UL)*(0x1000UL)) + ((((uint32_t) 0x40000000UL) + 0x00200000UL) + 0x00000UL));
    int u32ModChannId = ((i32ChannID)%16UL);
...
static int nu_pdma_timeout_set(int i32ChannID, int i32Timeout_us)
{
    ...
    PDMA = (PDMA_T *)((((i32ChannID)/16UL)*(0x1000UL)) + ((((uint32_t) 0x40000000UL) + 0x00200000UL) + 0x00000UL));

    u32ModChannId = ((i32ChannID)%16UL);

    nu_pdma_chn_arr[i32ChannID - (0)].m_u32IdleTimeout_us = i32Timeout_us;
    ...
    if (i32Timeout_us)
    {
        ...
        PDMA_EnableInt(PDMA, u32ModChannId, 0x00000004UL);
        ...
        if (u32ModChannId < 8)
            PDMA->TOUTPSC0_7 = (PDMA->TOUTPSC0_7 & ~(0x7ul << ((0) * u32ModChannId))) | (u32Divider << ((0) * u32ModChannId));
        else
            PDMA->TOUTPSC8_15 = (PDMA->TOUTPSC8_15 & ~(0x7ul << ((0) * (u32ModChannId % 8)))) | (u32Divider << ((0) * (u32ModChannId % 8)));
    ...
    {
        PDMA_DisableInt(PDMA, u32ModChannId, 0x00000004UL);
        PDMA_DisableTimeout(PDMA, 1 << u32ModChannId);
    }
...
int nu_pdma_channel_allocate(int32_t i32PeripType)
{
    ...
    for (j = (PDMA_START + 1); j < PDMA_CNT; j++)
    {
        ...
        if (ChnId < 16UL)
        {
            ...
            ChnId += (j * 16UL);
            memset(nu_pdma_chn_arr + ChnId - (0), 0x00, sizeof(nu_pdma_chn_t));
...
int nu_pdma_channel_free(int i32ChannID)
{
    ...
    if ((i32ChannID < (PDMA_CNT*16UL)) && (i32ChannID >= (0)))
    {
        nu_pdma_chn_mask_arr[((i32ChannID)/16UL)] &= ~(1 << ((i32ChannID)%16UL));
...
int nu_pdma_filtering_set(int i32ChannID, uint32_t u32EventFilter)
{
    ...
    nu_pdma_chn_arr[i32ChannID - (0)].m_u32EventFilter = u32EventFilter;
...
uint32_t nu_pdma_filtering_get(int i32ChannID)
{
    ...
    return nu_pdma_chn_arr[i32ChannID - (0)].m_u32EventFilter;
...
{
    int ret = 1;
    nu_pdma_chn_cb_t psChnCb_Current = 0;

    do { if(!(psChnCb != 0)) { printf("Asserted at expression: %s", "psChnCb != NULL"); while(1); } } while(0);
    ...
    {
        case eCBType_Event:
            psChnCb_Current = &nu_pdma_chn_arr[i32ChannID - (0)].m_sCB_Event;
            break;

        case eCBType_Trigger:
            psChnCb_Current = &nu_pdma_chn_arr[i32ChannID - (0)].m_sCB_Trigger;
            break;

        case eCBType_Disable:
            psChnCb_Current = &nu_pdma_chn_arr[i32ChannID - (0)].m_sCB_Disable;
            break;
...
nu_pdma_cb_handler_t nu_pdma_callback_hijack(int i32ChannID, nu_pdma_cbtype_t eCBType, nu_pdma_chn_cb_t psChnCb_Hijack)
{
    ...
    do { if(!(psChnCb_Hijack != 0)) { printf("Asserted at expression: %s", "psChnCb_Hijack != NULL"); while(1); } } while(0);

    sChnCB_Tmp.m_pfnCBHandler = 0;
...
{
    PDMA_T *PDMA = (PDMA_T *)((((i32ChannID)/16UL)*(0x1000UL)) + ((((uint32_t) 0x40000000UL) + 0x00200000UL) + 0x00000UL));
    return ((PDMA->DSCT[((i32ChannID)%16UL)].CTL & (0xfffful << (16))) >> (16)) + 1;
}
...
int nu_pdma_transferred_byte_get(int32_t i32ChannID, int32_t i32TriggerByteLen)
{
    ...
    i32BitWidth = PDMA->DSCT[((i32ChannID)%16UL)].CTL & (0x3ul << (12));
    i32BitWidth = (i32BitWidth == 0x00000000UL) ? 1 : (i32BitWidth == 0x00001000UL) ? 2 : (i32BitWidth == 0x00002000UL) ? 4 : 0;
...
nu_pdma_memctrl_t nu_pdma_channel_memctrl_get(int i32ChannID)
{
    ...
    eMemCtrl = nu_pdma_chn_arr[i32ChannID - (0)].m_spPeripCtl.m_eMemCtl;
...
int nu_pdma_channel_memctrl_set(int i32ChannID, nu_pdma_memctrl_t eMemCtrl)
{
    ...
    if ((psPdmaChann->m_spPeripCtl.m_u32Peripheral == 0UL) &&
            ((eMemCtrl == eMemCtl_SrcFix_DstInc) || (eMemCtrl == eMemCtl_SrcFix_DstFix)))
        goto exit_nu_pdma_channel_memctrl_set;

    nu_pdma_chn_arr[i32ChannID - (0)].m_spPeripCtl.m_eMemCtl = eMemCtrl;
...
static void nu_pdma_channel_memctrl_fill(nu_pdma_memctrl_t eMemCtl, uint32_t *pu32SrcCtl, uint32_t *pu32DstCtl)
{
    switch ((int)eMemCtl)
    {
        case eMemCtl_SrcFix_DstFix:
            *pu32SrcCtl = 0x00000300UL;
            *pu32DstCtl = 0x00000C00UL;
            break;

        case eMemCtl_SrcFix_DstInc:
            *pu32SrcCtl = 0x00000300UL;
            *pu32DstCtl = 0x00000000UL;
            break;

        case eMemCtl_SrcInc_DstFix:
            *pu32SrcCtl = 0x00000000UL;
            *pu32DstCtl = 0x00000C00UL;
            break;
...
{
    nu_pdma_periph_ctl_t *psPeriphCtl = 0;
    ...
    if (!dma_desc)
        ...
        goto exit_nu_pdma_desc_setup;
    else if (i32TransferCnt > (((0xfffful << (16))>>(16))+1))
        goto exit_nu_pdma_desc_setup;


    psPeriphCtl = &nu_pdma_chn_arr[i32ChannID - (0)].m_spPeripCtl;

    nu_pdma_channel_memctrl_fill(psPeriphCtl->m_eMemCtl, &u32SrcCtl, &u32DstCtl);

    dma_desc->CTL = ((i32TransferCnt - 1) << (16)) |
                    ((u32DataWidth == 8) ? 0x00000000UL : (u32DataWidth == 16) ? 0x00001000UL : 0x00002000UL) |
                    u32SrcCtl |
                    u32DstCtl |
                    0x00000001UL;
    ...
    if (psPeriphCtl->m_u32Peripheral == 0UL)
    {

        dma_desc->CTL |= (0x00000000UL | 0x00000020UL);
    }
    else
    {

        dma_desc->CTL |= (0x00000004UL);
    }

    if (next)
    {

        dma_desc->CTL = (dma_desc->CTL & ~(0x3ul << (0))) | 0x00000002UL;
        dma_desc->NEXT = (uint32_t)next;
    }


    if (u32BeSilent)
        dma_desc->CTL |= (0x1ul << (7));
...
static void nu_pdma_sgtbls_token_free(nu_pdma_desc_t psSgtbls)
{
    ...
    do { if(!(idx >= 0)) { printf("Asserted at expression: %s", "idx >= 0"); while(1); } } while(0);
    do { if(!((idx + 1) <= (16))) { printf("Asserted at expression: %s", "(idx + 1) <= NU_PDMA_SGTBL_POOL_SIZE"); while(1); } } while(0);
...
void nu_pdma_sgtbls_free(nu_pdma_desc_t *ppsSgtbls, int num)
{
    ...
    do { if(!(ppsSgtbls != 0)) { printf("Asserted at expression: %s", "ppsSgtbls != NULL"); while(1); } } while(0);
    do { if(!(num <= ((((0xfffful << (0))>>(0))+1)/sizeof(DSCT_T)))) { printf("Asserted at expression: %s", "num <= NU_PDMA_SG_TBL_MAXSIZE"); while(1); } } while(0);
...
int nu_pdma_sgtbls_allocate(nu_pdma_desc_t *ppsSgtbls, int num)
{
    ...
    do { if(!(ppsSgtbls)) { printf("Asserted at expression: %s", "ppsSgtbls"); while(1); } } while(0);
    do { if(!(num <= ((((0xfffful << (0))>>(0))+1)/sizeof(DSCT_T)))) { printf("Asserted at expression: %s", "num <= NU_PDMA_SG_TBL_MAXSIZE"); while(1); } } while(0);
...
static void _nu_pdma_transfer(int i32ChannID, uint32_t u32Peripheral, nu_pdma_desc_t head, uint32_t u32IdleTimeout_us)
{
    ...
    PDMA_DisableTimeout(PDMA, 1 << ((i32ChannID)%16UL));

    PDMA_EnableInt(PDMA, ((i32ChannID)%16UL), 0x00000001UL);
    ...
    if (u32Peripheral == 0UL)
    {
        PDMA_Trigger(PDMA, ((i32ChannID)%16UL));
    }
...
static void _nu_pdma_free_sgtbls(nu_pdma_chn_t *psPdmaChann)
{
    if (psPdmaChann->m_ppsSgtbl)
    {
        ...
        psPdmaChann->m_ppsSgtbl = 0;
        free(psPdmaChann->m_ppsSgtbl_unalign);
        psPdmaChann->m_ppsSgtbl_unalign = 0;
...
static int _nu_pdma_transfer_chain(int i32ChannID, uint32_t u32DataWidth, uint32_t u32AddrSrc, uint32_t u32AddrDst, uint32_t u32TransferCnt, uint32_t u32IdleTimeout_us)
{
    ...
    if (psPdmaChann->m_u32WantedSGTblNum != (u32TransferCnt / (((0xfffful << (16))>>(16))+1) + 1))
    {
        if (psPdmaChann->m_u32WantedSGTblNum > 0)
            _nu_pdma_free_sgtbls(psPdmaChann);

        psPdmaChann->m_u32WantedSGTblNum = u32TransferCnt / (((0xfffful << (16))>>(16))+1) + 1;
    ...
    {
        u32TxCnt = (u32TransferCnt > (((0xfffful << (16))>>(16))+1)) ? (((0xfffful << (16))>>(16))+1) : u32TransferCnt;

        ret = nu_pdma_desc_setup(i32ChannID,
                                 psPdmaChann->m_ppsSgtbl[i],
                                 u32DataWidth,
                                 (eMemCtl & 0x2ul) ? u32AddrSrc + u32Offset : u32AddrSrc,
                                 (eMemCtl & 0x1ul) ? u32AddrDst + u32Offset : u32AddrDst,
                                 u32TxCnt,
                                 ((i + 1) == psPdmaChann->m_u32WantedSGTblNum) ? 0 : psPdmaChann->m_ppsSgtbl[i + 1],
                                 ((i + 1) == psPdmaChann->m_u32WantedSGTblNum) ? 0 : 1);
...
int nu_pdma_transfer(int i32ChannID, uint32_t u32DataWidth, uint32_t u32AddrSrc, uint32_t u32AddrDst, uint32_t u32TransferCnt, uint32_t u32IdleTimeout_us)
{
    ...
    if (nu_pdma_check_is_nonallocated(i32ChannID))
        ...
        goto exit_nu_pdma_transfer;
    else if (u32TransferCnt > (((0xfffful << (16))>>(16))+1))
        return _nu_pdma_transfer_chain(i32ChannID, u32DataWidth, u32AddrSrc, u32AddrDst, u32TransferCnt, u32IdleTimeout_us);

    psPdmaChann = &nu_pdma_chn_arr[i32ChannID - (0)];
    psPeriphCtl = &psPdmaChann->m_spPeripCtl;

    head = &PDMA->DSCT[((i32ChannID)%16UL)];
...
void PDMA_IRQHandler(PDMA_T *PDMA)
{
    ...
    uint32_t intsts = ((uint32_t)(PDMA->INTSTS));
    uint32_t abtsts = ((uint32_t)(PDMA->ABTSTS));
    uint32_t tdsts = ((uint32_t)(PDMA->TDSTS));
    uint32_t unalignsts = ((uint32_t)(PDMA->ALIGN));
    uint32_t reqto = intsts & (((1ul<<16UL)-1) << (8));
    uint32_t reqto_ch = (reqto >> (8));
    ...
    if (intsts & (0x1ul << (2)))
    {
        ...
        ((uint32_t)(PDMA->ALIGN = (unalignsts)));
    }
    ...
    while ((i = nu_ctz(allch_sts)) < 16UL)
    {
        int module_id = ((uint32_t)PDMA - ((((uint32_t) 0x40000000UL) + 0x00200000UL) + 0x00000UL)) / (0x1000UL);
        int j = i + (module_id * 16UL);
        int ch_mask = (1 << i);

        if (nu_pdma_chn_mask_arr[module_id] & ch_mask)
        {
            int ch_event = 0;
            nu_pdma_chn_t *dma_chn = nu_pdma_chn_arr + j - (0);
            ...
            if (dma_chn->m_sCB_Event.m_pfnCBHandler)
            {
                ...
                if (reqto_ch & ch_mask)
                    nu_pdma_timeout_set(j, nu_pdma_chn_arr[j - (0)].m_u32IdleTimeout_us);
...
{
    PDMA_IRQHandler(((PDMA_T *) ((((uint32_t) 0x40000000UL) + 0x00200000UL) + 0x00000UL)));
}

void PDMA1_IRQHandler(void)
{
    PDMA_IRQHandler(((PDMA_T *) ((((uint32_t) 0x40000000UL) + 0x00200000UL) + 0x01000UL)));
}
...
static void nu_pdma_memfun_actor_init(void)
{
    ...
    for (i = 0; i < (4); i++)
    {
        ...
        if (-(1) != (nu_pdma_memfun_actor_arr[i].m_i32ChannID = nu_pdma_channel_allocate(0UL)))
        {
...
static int nu_pdma_memfun(void *dest, void *src, uint32_t u32DataWidth, unsigned int u32TransferCnt, nu_pdma_memctrl_t eMemCtl)
{
    ...
    nu_pdma_memfun_actor_t psMemFunActor = 0;
    ...
    nu_pdma_filtering_set(psMemFunActor->m_i32ChannID, (1 << 0) | (1 << 1));
    ...
    if (psMemFunActor->m_u32Result & (1 << 1))
    {
        ret += u32TransferCnt;
    }
    else
    ...
    if (psMemFunActor->m_u32Result & (1 << 0))
    {
